...
The limesurvey-saml-0.2 works with limesurvey 3.x
...
1. SAML ONLY
    ...
    When the user access to the login page, he will be redirected to the IdP. After authenticate:
      ...
      a) It checks the user's groups to see if they are allowed to use LimeSurvey.  
      b) If is the first time, the user account will be created (stored in the file)
...
Author Version 0.2: Axel Naumann <axel.naumann@fn.de> 
 ...
 * saml_group_mapping: SAML attributed used for groups
 
 * check_user_group: Enable it when you want to check the User Group
 
 * user_access_group: Name of the User Access Group
